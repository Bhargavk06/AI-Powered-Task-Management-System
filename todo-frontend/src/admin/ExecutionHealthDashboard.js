  ...
  const fetchHealthData = async () => {
    try {
      ...
      const endpoints = [
        ...
        `${backendUrl}/api/health/tasks-by-window`,
        `${backendUrl}/api/health/completion-trend`
      ];
      ...
      // Check all responses
      const [healthRes, windowRes, trendRes] = responses;
      
      if (!healthRes.ok) {
        const text = await healthRes.text();
        console.error('Health endpoint error:', healthRes.status, text);
        throw new Error(`Health API returned ${healthRes.status}: ${healthRes.statusText}`);
      }
      
      if (!windowRes.ok) {
        const text = await windowRes.text();
        console.error('Window endpoint error:', windowRes.status, text);
        throw new Error(`Tasks window API returned ${windowRes.status}: ${windowRes.statusText}`);
      }
      
      if (!trendRes.ok) {
        const text = await trendRes.text();
        console.error('Trend endpoint error:', trendRes.status, text);
        throw new Error(`Trend API returned ${trendRes.status}: ${trendRes.statusText}`);
      }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4">
        <div className="text-center bg-white p-8 rounded-lg shadow-lg max-w-md">
          ...
          <p className="text-red-600 text-lg font-semibold mb-2">Failed to Load Health Metrics</p>
